...
class GameController: 
    ...
    def startListeners(self):
        ...
        # Join all threads (this will block forever, but that's okay for listening)
        for thread in threads:
            thread.join()
    
   
    def decode(self, data):
        header, version, packetNumber, playersPerTeam, competitionPhase, competitionType, gamePhase, state, setPlay, firstHalf, kickingTeam, secsRemaining, secondaryTime = struct.unpack_from('4s10B2h', data)
    ...
    def decodeTeam(self, data, offset):
        teamNumber, fieldPlayerColour, goalkeeperColour, goalkeeper, score, penaltyShot, singleShots, messageBudget = struct.unpack_from('6B2H', data, offset)
